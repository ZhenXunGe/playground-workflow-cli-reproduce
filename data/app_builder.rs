...
fn read_inputs_from_file(filename: String) -> Vec<String> {
    use std::fs::File;
    use std::io::BufRead;
...
impl ArgBuilder<Vec<String>> for PrivateInputsFilenameArg {
    fn builder() -> Arg<'static> {
        arg!(--private_file <PRIVATE_INPUT_FILENAME> ... "Filename containing private inputs")
...
fn dry_run_command() -> Command<'static> {
    Command::new("dry-run")
        ...
        .arg(PrivateInputsFilenameArg::builder())
        .arg(ContextInputsFilenameArg::builder())
...
impl From<&ArgMatches> for RunningArg {
    fn from(val: &ArgMatches) -> Self {
        RunningArg {
            ...
            private_inputs: PrivateInputsFilenameArg::parse(val),
            context_inputs: ContextInputsFilenameArg::parse(val),
